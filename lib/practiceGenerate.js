// Practice item generation — composes the prompt, calls Anthropic, validates the result, retries
// once with the validation error appended to the prompt. Used by api/practice.js (with real
// Supabase-sourced promptContext) and by the practice test suite (with fixture promptContext) —
// both call this exact function so tests exercise the real generation logic, not a
// re-implementation of it.
//
// Every mode forces tool_choice so the response is schema-conformant by construction. mc_cloze
// additionally runs with extended (adaptive) thinking on — a real generated sentence, not just a
// routed classification, benefits from it more than the other two problem types do, and its task
// prompt (lib/practiceRules.js) leans on that scratch space explicitly: draft the correct sentence
// there first — checking for itself that the target skill is load-bearing — before turning it into
// the cloze via the tool call. (Forcing a specific tool ordinarily suppresses any plain-text
// preamble the model might otherwise write — an earlier version of this function tried to work
// around that by leaving tool_choice `auto` on mc_cloze's first attempt so the model could draft
// in visible text, but that made the model skip the tool call outright often enough to matter;
// thinking blocks are exempt from that suppression, so forcing the tool while thinking is on gets
// the same "draft first" benefit without that failure mode.) The full response (thinking included)
// is handed back via `onResponse` purely for audit logging (see api/practice.js) — the UI is only
// ever shown the parsed `item`, never any of that.
    ...
    input_schema: {
      ...
      properties: {
        ...
        option_meanings: {
          type: 'array',
          items: { type: 'string' },
          description: 'One short English gloss per option, same order and same length as `options` — required for every card, not just vocabulary ones (for a grammar/expression card, gloss what each option itself means/does, not the target skill).',
        },
        used_seed_words: {
          type: 'array',
          items: { type: 'string' },
      ...
      required: ['sentence', 'options', 'answer', 'translation', 'option_meanings'],
    },
  },
  spelling: {
...
// `onResponse`, if given, is called with the raw response content array (message.content) right
// after each Anthropic call — same "overwritten each attempt, final one wins" semantics as
// onRequest. For mc_cloze this is the only place the model's `thinking` block (its sentence-drafting
// reasoning ahead of the tool call — see lib/practiceRules.js) is ever exposed; api/practice.js both
// console.logs it server-side (so it's visible in the backend, not just billed silently) and rounds
// the whole response back to the client purely for conversation-history logging, never for display.
...
export async function generatePracticeItem({ anthropic, model = DEFAULT_PRACTICE_MODEL, checkModel = DEFAULT_CHECK_MODEL, mode, promptContext, history = [], onRequest, onResponse, onCheck, onUsage, onCheckUsage }) {
  const promptId = PROMPT_IDS[mode]
  ...
  async function generate(retryNote, previousItem) {
    ...
    // Extended thinking, mc_cloze only: `{ type: 'adaptive' }` is the current (non-deprecated) way
    // to turn it on for this app's model family (Sonnet/Opus 4.6+) — the old fixed `budget_tokens`
    // form is a transitional escape hatch, not something to write in new code. `display: 'summarized'`
    // is pinned explicitly (rather than relying on the model's own default, which varies by model
    // and could go silent on a future PRACTICE_MODEL swap) so the thinking block always carries
    // readable text — see onResponse below for where it's surfaced. tool_choice stays forced even
    // with thinking on: that combination is fine on the direct Claude API (only Bedrock requires
    // disabling thinking to force a tool call), and forcing is what makes the thinking block do its
    // job — see the top-of-file comment for why an unforced `auto` choice was tried and dropped.
    // `max_tokens` is bumped well above the other modes' 1024 since thinking tokens share the cap.
    const thinkingEnabled = mode === 'mc_cloze'
    const message = await anthropic.messages.create({
      ...
      ...(thinkingEnabled ? { thinking: { type: 'adaptive', display: 'summarized' } } : {}),
    })
    ...
    if (!toolUse) throw new PracticeValidationError(`Model did not return a tool call (stop_reason: ${message.stop_reason})`)
