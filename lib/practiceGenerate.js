  ...
  mc_cloze: {
    ...
    input_schema: {
      ...
      properties: {
        ...
        frame: {
          type: 'string',
          description: 'Only for a skill that drills which preposition a verb/construction governs: the specific verb, adjective, or fixed construction you chose FIRST — before writing the sentence — that genuinely, idiomatically governs the target preposition in this exact use-case (e.g. "sich erinnern an", not just any verb that could loosely take "an"). Copied verbatim as it appears worked into the sentence. Omit entirely for every other skill/problem type.',
        },
        distractor_reasons: {
          type: 'array',
          items: {
...
export async function generatePracticeItem({ anthropic, model = DEFAULT_PRACTICE_MODEL, checkModel = DEFAULT_CHECK_MODEL, mode, promptContext, history = [], onRequest, onCheck }) {
  const promptId = PROMPT_IDS[mode]
  ...
  async function attempt(retryNote, previousItem) {
    ...
    if (mode === 'mc_cloze') {
      const verification = await verifyMcClozeItem({ anthropic, model: checkModel, lang, item })
