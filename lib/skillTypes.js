...
//
// Gate on a *stable* threshold, not a bare number — a prerequisite skill whose level clears
// minLevel but is currently `learning` or `relearning` does NOT satisfy the gate, so a lapsed
// prerequisite (any wrong answer drops a skill straight to `learning`, see
// lib/practiceScheduling.js) can't keep unlocking dependents while it's still being re-proven.
