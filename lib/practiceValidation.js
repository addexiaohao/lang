...
export function validatePracticeItem(mode, item, { cardKind, cardTags, skillType } = {}) {
  ...
  if (mode === 'mc_cloze') {
    ...
    if (skillType === 'production' && cardTags?.includes('production-which-conjunction') && (typeof item.frame !== 'string' || !item.frame.trim())) {
      throw new PracticeValidationError('frame is required for this skill — state the specific temporal/causal/concessive relationship this conjunction expresses, before the sentence')
    }
    return
  }
