...
import { logLlmApiCall } from './llmUsageLog.js'
...
// projectId/userId/cardId are purely for the llm_api_call log (this file already has a hard
// Supabase dependency via sense_check_cache below, unlike the DB-free generation/check libs, so
// logging directly here rather than through an onUsage callback is fine) — omit them and the row
// just logs with null project/user/card.
export async function checkSense({ anthropic, model = DEFAULT_SENSE_CHECK_MODEL, lemma, excerpt, languageName, projectId = null, userId = null, cardId = null }) {
  ...
  try {
    ...
    await logLlmApiCall({
      projectId, userId, purpose: 'sense_check', model, cardId,
...
export async function getSenseVerdict({ anthropic, model, projectId, lemma, excerpt, languageName, userId = null, cardId = null }) {
  ...
  const result = await checkSense({ anthropic, model, lemma, excerpt, languageName, projectId, userId, cardId })
...
export async function suggestSense({ anthropic, model = DEFAULT_SENSE_CHECK_MODEL, card, excerpts = [], languageName, projectId = null, userId = null, cardId = null }) {
  const lang = languageName ?? 'the target language'
  const context = [
    `Language: ${lang}`,
    `Word: ${card.name}`,
  ...
  try {
    ...
    await logLlmApiCall({
      projectId, userId, purpose: 'suggest_sense', model, cardId,
      status: 'error', errorMessage: e.message, latencyMs: Date.now() - startedAt,
    })
    throw e
  }
