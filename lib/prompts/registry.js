...
function skillSection(skillType, extraPrompt) {
  if (!skillType) return ''
  // `extraPrompt` was previously dropped entirely here — silently discarding the "meaning" row's
  // self-check bullets (lib/practiceRules.js), the sense-drilling note (api/practice.js's
  // senseGloss branch), and Card Groups' discrimination instructions alike. All three depend on
  // this text actually reaching the model, so it's included the same way the non-"meaning" branch
  // below already does.
...
// Closed-vocabulary policy — plan.md §7's most aggressive lever for cutting practice-sentence
// difficulty: a hard constraint on the vocabulary itself, not just a seed list of words the model
// MAY use. The actual permitted word list (thousands of entries) is never sent — only the policy
// and its fallback rule, so the model self-polices rather than being handed a list to pick from.
function vocabularyPolicy() {
