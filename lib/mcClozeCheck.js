...
// reason "well it's not X or Y so it must be Z." `onUsage`, if given, fires once with
// { model, usage, stopReason, latencyMs } after the call — lets a DB-backed caller (api/practice.js,
// via generatePracticeItem's onCheckUsage) log every check to llm_api_call without this function
// itself depending on Supabase (this file is also driven by the DB-free practice test suites).
export async function checkSentence({ anthropic, model, lang, sentence, onUsage }) {
...
export async function verifyMcClozeItem({ anthropic, model = DEFAULT_CHECK_MODEL, lang, item, onUsage }) {
  const checks = await Promise.all(item.options.map(async (option) => {
    ...
    const result = await checkSentence({ anthropic, model, lang, sentence, onUsage })
