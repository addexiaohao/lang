...
// options.isGateSatisfied: (card, externalType, dbTypedSiblingRows) => bool — the resolved
//   LanguagePack's gate check (api/skills.js passes pack.isGateSatisfied). Defaults to "no gates"
//   so the one caller that forgets simply gets ungated selection rather than a crash.
//
...
export function selectScheduledPracticeSkills(cards, n, options = {}) {
  const { now = new Date(), failureCap = FAILURE_CAP, isGateSatisfied = () => true } = options
