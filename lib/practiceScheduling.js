...
// The manual editor (CardDetailPanel/SkillsPanel's PATCH .../knowledge-cards { level }) isn't an
// outcome, so it never applies the correct/incorrect transition table above. It only needs to
// handle: retiring (level set to 10), un-setting (level cleared to null), un-retiring (a hand-set
// level given to a currently-retired skill re-enters at 'learning', since there's no prior
// qualitative state left to resume), and — because interval_days is a pure function of level —
// recomputing due_at/interval_days to match whatever level was just hand-set, for every other case.
// A hand-set level is a self-assessment, not a proof of recall, so it never jumps a skill straight
// to 'stable' on its own; it also never knocks an already-'stable' skill back down to 'learning' —
// correcting a level number isn't an incorrect answer.
export function scheduleForManualLevel(current, level) {
  if (level === RETIRED_LEVEL) return { ...RETIRED_SCHEDULE }
  if (level === null) return { ...NEVER_SCHEDULE }
  const state = current.state === 'retired' || current.state === 'never' ? 'learning' : current.state
  const interval = intervalForLevel(level)
