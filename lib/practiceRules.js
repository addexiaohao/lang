  ...
  production: [
    {
      problemType: 'mc_cloze', weight: () => 1, prompt: ({ cardName, cardTags }) => {
        // Not every `production` card is about which preposition to use (see SKILL_TYPES.grammar —
        // 'production' is grammar's only flat type) — the frame-first instructions below only make
        // sense, and are only forced (see lib/practiceValidation.js's `frame` requirement), for
        // cards actually tagged `production-which-preposition` (see tests/fixtures/cards.js).
